...
import org.example.part3.client.ServiceCenter;
import org.example.part3.client.ZKServiceCenter;
import org.example.part3.common.RpcClient;
import org.example.part3.common.RpcRequest;
...
public class NettyRpcClient implements RpcClient {
    ...
    public RpcResponse sendRequest(RpcRequest request) {
        InetSocketAddress inetSocketAddress = serviceCenter.serviceDecovery(request.getInterfaceName());
        String addresss = inetSocketAddress.getHostName();
        int port1 = inetSocketAddress.getPort();
        try {
            // 创建一个channelFuture对象，代表这一个操作时间，sync方法表示堵塞直到connect完成
            ChannelFuture channelFuture = bootstrap.connect(addresss, port1).sync();
            // channel 表示一个连接的单位
